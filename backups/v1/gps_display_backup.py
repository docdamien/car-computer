...
def checkForFix():
    ...
    with canvas(device) as draw:
        draw.text((0, 48), "looking for fix", fill=255)
    ...
    while True:
            ...
            if b"+CGNSINF: 1,1," in response:
                ...
                device.clear()
                with canvas(device) as draw:
                    draw.text((0, 48), "fix found", fill=255)
            ...
            if b"+CGNSINF: 1,0," in response:
                ...
                device.clear()
                with canvas(device) as draw:
                    draw.text((0, 48), "still looking......", fill=255)
            else:
                ser.write(b"AT+CGNSINF\r")
        ...
        for i in range(10):
            ...
            if checkForFix():
                ...
                if getCoord():
                    ...
                    print ("Time:", time)
                    print ("Step", i+1, "out of 10")
                    
                    device.clear()
                    with canvas(device) as draw:
                        draw.text((0, 0), "Current coordinates:", fill=255)
                        draw.text((0, 8), coord, fill=255)
                        draw.text((0, 24), "Current speed:", fill=255)
                        draw.text((0, 32), speed, fill=255)
            ...
            if i == 9:
                ...
                device.clear()
                with canvas(device) as draw:
                    draw.text((0, 48), "... streaming", fill=255)
                ...
                if openPPPD():
                    ...
                    device.clear()
                    with canvas(device) as draw:
                        draw.text((0, 48), "streaming done", fill=255)
